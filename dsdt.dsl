 ...
 * 
 * Disassembly of dsdt.dat, Mon Jul 13 23:30:46 2015
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001938E (103310)
 *     Revision         0x02
 *     Checksum         0xBE
 *     OEM ID           "_ASUS_"
...
DefinitionBlock ("dsdt.aml", "DSDT", 2, "_ASUS_", "Notebook", 0x01072009)
{
    ...
    External (_SB_.PCI0.GFX0.OPTS, MethodObj)    // Warning: Unresolved Method, guessing 4 arguments (may be incorrect, see warning above)
    ...
    External (_SB_.PCI0.RP05.PEGP.DWBL, MethodObj)    // Warning: Unresolved Method, guessing 0 arguments (may be incorrect, see warning above)
    External (_SB_.PCI0.RP05.PEGP.SWHD, MethodObj)    // Warning: Unresolved Method, guessing 1 arguments (may be incorrect, see warning above)
    External (_SB_.PCI0.RP05.PEGP.UPBL, MethodObj)    // Warning: Unresolved Method, guessing 0 arguments (may be incorrect, see warning above)
    ...
    External (_SB_.IAOE.IMDS)
    External (_SB_.IAOE.ISEF)
    ...
    External (_SB_.PCI0.RP05.PEGP.LCDD._DCS)
    ...
    Name (VGAF, One)
    OperationRegion (GNVS, SystemMemory, 0xCB770000, 0x0359)
    ...
    Scope (_SB)
    {
        Device (PCI0)
        {
            ...
            Device (LPCB)
            {
                ...
                OperationRegion (CPSB, SystemMemory, 0xCAA65F18, 0x10)
    ...
    Scope (_SB.PCI0.I2C1)
    {
        Device (ACD0)
        {
            ...
            Name (_SUB, "1043183D")  // _SUB: Subsystem ID
    ...
    Scope (\)
    {
        ...
        Device (DFUD)
        {
            ...
            Method (_DSM, 4, Serialized)  // _DSM: Device-Specific Method
            {
                ...
                If (LEqual (Arg0, Buffer (0x10)
                        {
                            /* 0000 */   0x1C, 0x83, 0x30, 0x56, 0xC9, 0x06, 0x56, 0x48,
                            /* 0008 */   0xB3, 0x27, 0xF5, 0xD3, 0x25, 0x86, 0xE0, 0x60
                        }))
                {
    ...
    Method (RPTS, 1, NotSerialized)
    {
        ...
        If (LAnd (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)), And (
            ICNF, One)))
        {
                    ...
                    Else
                    {
                        If (LEqual (Arg0, 0x03))
                        {
                            If (ALAE)
                            {
                                ...
                                Else
                                {
                                    If (LAnd (LLessEqual (Local0, 0x012C), LGreater (Local0, 0x64)))
                                    {
                                        Return (One)
                                    }
                                    Else
                                    {
                                        If (LGreater (Local0, 0x012C))
                                        {
                                            Return (0x02)
                                        }
                                        Else
    ...
    Scope (\)
    {
        ...
        Method (OEMS, 1, NotSerialized)
        {
            ...
            \_SB.PCI0.GFX0.OPTS (Arg0, If (LEqual (Arg0, 0x03))
                {
                    If (LLessEqual (MSOS (), OSME))
                    {
                        Store (One, \_SB.WIDE)
                ...
                }, SBRS (Arg0), \_SB.PCI0.LPCB.EC0.EC0S (Arg0))
            Store (Arg0, \_SB.SLPN)
            DIAG (Add (Arg0, 0xD0))
            PPRJ (Arg0)
            Store (Arg0, \_SB.SLPT)
    ...
    Scope (_SB.PCI0.LPCB.EC0)
    {
        ...
        Method (_QCD, 0, NotSerialized)  // _Qxx: EC Query
        {
            ...
            If (LNotEqual (OLUX, Local0))
            {
                ...
                {
                    Store (Zero, Local2)
                    If (LAnd (LLessEqual (OLUX, 0x64), LGreater (Local0, 0x64)))
                    {
                        Store (One, Local2)
                    }
                    Else
                    {
                        If (LAnd (LLessEqual (OLUX, 0x012C), LGreater (Local0, 0x012C)))
                        {
                            Store (One, Local2)
                        }
                        Else
                        {
                            If (LAnd (LGreaterEqual (OLUX, 0x012C), LLess (Local0, 0x012C)))
                            {
                                Store (One, Local2)
                            }
                            Else
                            {
                                If (LAnd (LGreaterEqual (OLUX, 0x64), LLess (Local0, 0x64)))
                                {
                                    Store (One, Local2)
                                }
                            }
    ...
    Scope (\)
    {
        ...
        Method (OPLD, 1, Serialized)
        {
            ...
            Store (Or (And (Local0, 0x7F), ShiftLeft (And (Local2, One
                ), 0x07)), Index (BPLD, 0x0A))
            Store (Or (And (Local1, 0x80), ShiftRight (And (Local2, 0xFE
                ), One)), Index (BPLD, 0x0B))
        }
    }
    ...
    If (CondRefOf (\_SB.PCI0.XHC.RHUB.SSP4))
    {
        Scope (_SB.PCI0.XHC.RHUB.SSP4)
        {
            ...
            Device (CDPT)
            {
                Method (_STA, 0, Serialized)  // _STA: Status
                {
                    If (LOr (LAnd (LEqual (BID, 0x31), LEqual (SKID, One)), LEqual (
                        BID, 0x80)))
                    {
                        Return (0x0F)
                    }
    ...
    OperationRegion (ABNV, SystemMemory, 0xCB75F000, 0x000D)
    ...
    Device (_SB.TPM)
    {
        ...
        Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
        {
            If (LEqual (TTPF, One))
            {
                CreateDWordField (CRST, \_SB.TPM._Y2D._BAS, MTFD)  // _BAS: Base Address
                CreateDWordField (CRST, \_SB.TPM._Y2D._LEN, LTFD)  // _LEN: Length
